...
export default function ChallengeEnrollments({ enrollments, filters }) {
    ...
    const challengeColors = {
        'Story Challenge': 'bg-orange-500/15 text-orange-400',
        'Poetry Challenge': 'bg-purple-500/15 text-purple-400',
        'Academic Challenge': 'bg-cyan-500/15 text-cyan-400',
    };
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                        <div>
                            <Link
                                ...
                                className="inline-flex items-center gap-2 text-gray-500 hover:text-white text-sm font-medium mb-3 transition-colors"
                            >
                        ...
                        <div className="flex gap-3">
                            <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-center">
                                <p className="text-2xl font-black text-white">{enrollments.total || 0}</p>
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-6">
                        <div className="flex flex-col md:flex-row gap-4 items-center">
                            ...
                            <form onSubmit={handleSearch} className="flex-1 w-full">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-black/30 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-indigo-500"
                                    />
                                </div>
                            </form>
                            ...
                            <select
                                ...
                                className="bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500"
                            >
                                    ...
                                    {enrollments.data && enrollments.data.length > 0 ? (
                                        enrollments.data.map((enrollment) => (
                                            <tr key={enrollment.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <p className="text-white font-semibold text-sm">{enrollment.full_name}</p>
                                                </td>

                                                {/* Contact */}
                                                <td className="px-4 py-4">
                                                    <p className="text-gray-300 text-sm">{enrollment.email}</p>
                                                    <p className="text-gray-500 text-xs">{enrollment.mobile_number}</p>
                                                </td>

                                                {/* City */}
                                                <td className="px-4 py-4">
                                                    <p className="text-gray-300 text-sm">{enrollment.city}</p>
                                                </td>

                                                {/* Challenge Type */}
                                                <td className="px-4 py-4">
                                                    <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${challengeColors[enrollment.challenge_type] || 'bg-gray-500/15 text-gray-400'}`}>
                                                ...
                                                {/* Fee */}
                                                <td className="px-4 py-4">
                                                    <p className="text-white font-bold text-sm">₹{parseFloat(enrollment.entry_fee || 1999).toLocaleString()}</p>
                                                </td>
                                                    ...
                                                    {editingId === enrollment.id ? (
                                                        <div className="space-y-2 min-w-[180px]">
                                                            <select
                                                                ...
                                                                className="w-full bg-black/50 border border-white/20 rounded-lg px-3 py-1.5 text-white text-xs focus:outline-none focus:border-indigo-500"
                                                            >
                                                                <option value="pending">Pending</option>
                                                                <option value="paid">Paid</option>
                                                                <option value="failed">Failed</option>
                                                            ...
                                                            <textarea
                                                                ...
                                                                className="w-full bg-black/50 border border-white/20 rounded-lg px-3 py-1.5 text-white text-xs focus:outline-none focus:border-indigo-500 resize-none"
                                                                rows="2"
                                                            />
                                                            <div className="flex gap-2">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="px-3 py-1 bg-white/10 hover:bg-white/20 text-gray-300 text-xs font-bold rounded-lg transition-colors"
                                                                >
                                                                    Cancel
                                                                </button>
                                                            </div>
                                                        ...
                                                        <div>
                                                            ...
                                                            {enrollment.admin_notes && (
                                                                <p className="text-gray-500 text-xs mt-1 max-w-[150px] truncate" title={enrollment.admin_notes}>
                                                                    📝 {enrollment.admin_notes}
                                                                </p>
                                                            )}
                                                        </div>
                                        ...
                                        <tr>
                                            <td colSpan="8" className="px-6 py-16 text-center">
                                                ...
                                                <p className="text-gray-600 text-sm mt-1">Challenge enrollments will appear here</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
