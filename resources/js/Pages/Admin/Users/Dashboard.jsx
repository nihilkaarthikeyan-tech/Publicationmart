            ...
            <div className="min-h-screen bg-parchment py-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center">
                                ...
                                <div>
                                    <h1 className="text-3xl font-bold text-ink">{user.name}'s Dashboard</h1>
                                    <p className="text-umber mt-1">{user.email}</p>
                                </div>
                            </div>
                            <Link
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        ...
                        <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 text-ink">
                    ...
                    <div className="bg-vellum backdrop-blur-xl rounded-2xl border border-linen p-6 mb-8">
                        <h2 className="text-xl font-bold text-ink mb-6">Sales Channel Breakdown</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-paper rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-orange-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-ink font-bold">Amazon</h4>
                                <p className="text-2xl font-bold text-orange-800">{analytics.breakdown?.amazon?.quantity || 0}</p>
                                <p className="text-umber text-sm">₹{analytics.breakdown?.amazon?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                            <div className="bg-paper rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-blue-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-ink font-bold">Google Play</h4>
                                <p className="text-2xl font-bold text-blue-700">{analytics.breakdown?.google?.quantity || 0}</p>
                                <p className="text-umber text-sm">₹{analytics.breakdown?.google?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                            <div className="bg-paper rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-green-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-ink font-bold">Direct Sales</h4>
                                <p className="text-2xl font-bold text-green-700">{analytics.breakdown?.direct?.quantity || 0}</p>
                                <p className="text-umber text-sm">₹{analytics.breakdown?.direct?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                            <div className="bg-paper rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-purple-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-ink font-bold">International</h4>
                                <p className="text-2xl font-bold text-purple-700">{analytics.breakdown?.other?.quantity || 0}</p>
                                <p className="text-umber text-sm">₹{analytics.breakdown?.other?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-vellum backdrop-blur-xl rounded-2xl border border-linen overflow-hidden">
                        <div className="p-6 border-b border-linen">
                            <h2 className="text-xl font-bold text-ink">{user.name}'s Books</h2>
                        </div>
                        ...
                        {books && books.length > 0 ? (
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead className="bg-paper">
                                        <tr>
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-umber uppercase tracking-wider">Title</th>
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-umber uppercase tracking-wider">Author Name</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-umber uppercase tracking-wider">Status</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-umber uppercase tracking-wider">Price</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-umber uppercase tracking-wider">Created</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-umber uppercase tracking-wider">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-linen">
                                        {books.map((book) => (
                                            <tr key={book.id} className="hover:bg-paper transition-colors">
                                                <td className="px-6 py-4">
                                                    <p className="text-ink font-medium">{book.title}</p>
                                                </td>
                                                <td className="px-6 py-4 text-ink-soft">{book.author_name}</td>
                                                ...
                                                <td className="px-6 py-4 text-center text-ink font-medium">
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="text-ink font-medium">No books yet</h3>
                                <p className="text-umber text-sm mt-1">This author hasn't created any books</p>
                            </div>
                        )}
                    </div>
