                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center">
                                ...
                                <div>
                                    <h1 className="text-3xl font-bold text-white">{user.name}'s Dashboard</h1>
                                    <p className="text-gray-400 mt-1">{user.email}</p>
                                </div>
                            </div>
                            <Link
                                href={backUrl || route('admin.users.index')}
                                className="px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all border border-white/10 flex items-center gap-2"
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white">
                            <h3 className="text-blue-100 text-sm font-medium">Total Books</h3>
                        ...
                        <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 text-white">
                            <h3 className="text-green-100 text-sm font-medium">Total Sales</h3>
                            <p className="text-4xl font-bold mt-2">{analytics.totalSales}</p>
                            <div className="mt-2 text-green-200 text-sm">copies sold</div>
                        </div>
                        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white">
                            ...
                            <div className="mt-2 text-purple-200 text-sm">gross earnings</div>
                        </div>
                        <div className="bg-gradient-to-br from-orange-500 to-amber-600 rounded-2xl p-6 text-white">
                            <h3 className="text-orange-100 text-sm font-medium">Total Royalty</h3>
                            <p className="text-4xl font-bold mt-2">₹{analytics.totalRoyalty?.toFixed(2) || '0.00'}</p>
                            <div className="mt-2 text-orange-200 text-sm">author earnings</div>
                        </div>
                    </div>

                    {/* Sales Channel Breakdown */}
                    <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/10 p-6 mb-8">
                        <h2 className="text-xl font-bold text-white mb-6">Sales Channel Breakdown</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-white/5 rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-orange-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-white font-bold">Amazon</h4>
                                <p className="text-2xl font-bold text-orange-400">{analytics.breakdown?.amazon?.quantity || 0}</p>
                                <p className="text-gray-400 text-sm">₹{analytics.breakdown?.amazon?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                            <div className="bg-white/5 rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-blue-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-white font-bold">Google Play</h4>
                                <p className="text-2xl font-bold text-blue-400">{analytics.breakdown?.google?.quantity || 0}</p>
                                <p className="text-gray-400 text-sm">₹{analytics.breakdown?.google?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                            <div className="bg-white/5 rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-green-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-white font-bold">Direct Sales</h4>
                                <p className="text-2xl font-bold text-green-400">{analytics.breakdown?.direct?.quantity || 0}</p>
                                <p className="text-gray-400 text-sm">₹{analytics.breakdown?.direct?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                            <div className="bg-white/5 rounded-xl p-4 text-center">
                                <div className="w-3 h-3 bg-purple-500 rounded-full mx-auto mb-2"></div>
                                <h4 className="text-white font-bold">International</h4>
                                <p className="text-2xl font-bold text-purple-400">{analytics.breakdown?.other?.quantity || 0}</p>
                                <p className="text-gray-400 text-sm">₹{analytics.breakdown?.other?.revenue?.toFixed(2) || '0.00'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
                        <div className="p-6 border-b border-white/10">
                            <h2 className="text-xl font-bold text-white">{user.name}'s Books</h2>
                        </div>
                        ...
                        {books && books.length > 0 ? (
                                ...
                                <table className="w-full">
                                    <thead className="bg-white/5">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Author Name</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider">Price</th>
                                            <th className="px-6 py-4 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider">Created</th>
                                    ...
                                    <tbody className="divide-y divide-white/10">
                                        {books.map((book) => (
                                            <tr key={book.id} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-gray-300">{book.author_name}</td>
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="text-white font-medium">No books yet</h3>
                                <p className="text-gray-400 text-sm mt-1">This author hasn't created any books</p>
                            </div>
                        )}
                    </div>
