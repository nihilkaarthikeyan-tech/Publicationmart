...
export default function GuestCheckout({ plan, price, pages }) {
    ...
    const accentText = isPremium ? 'text-cyan-400' : 'text-indigo-400';
            ...
            <div className="min-h-screen bg-[#0f0a1e] text-white font-sans flex items-center justify-center p-4">
                ...
                <div className="w-full max-w-5xl relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-start">
                    ...
                    <div className="space-y-8 lg:sticky lg:top-10">
                        <div>
                            <h1 className="text-4xl font-black text-white mb-2 tracking-tight">Setup Your Book</h1>
                            <p className="text-gray-400">Enter your details to create your writing workspace.</p>
                        </div>

                        <div className="bg-[#0d1220] rounded-3xl border border-white/10 p-8 relative overflow-hidden group">
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-gray-400 text-sm font-medium uppercase tracking-wider">Selected Plan</h3>
                                {isPremium && (
                                    <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-cyan-500/20 text-cyan-400 border border-cyan-500/30">
                                        PREMIUM
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="flex justify-between items-end mb-6">
                                <div>
                                    <h2 className="text-3xl font-bold text-white capitalize">{plan} Plan</h2>
                                    <p className={`${accentText} font-medium`}>{pages} Pages</p>
                                </div>
                                <div className="text-right">
                                    <span className="text-3xl font-black text-white">₹{Number(price).toLocaleString('en-IN')}</span>
                                </div>
                            </div>
                        ...
                        <div className={`${glowBg} border ${glowBorder} rounded-2xl p-6 flex gap-4 items-start`}>
                            ...
                            <div>
                                <h4 className="font-bold text-white text-sm mb-1">Secure Payment via PhonePe</h4>
                                <p className="text-gray-400 text-xs leading-relaxed">Your payment is processed securely. We do not store your card details. Redirecting to secure gateway upon confirmation.</p>
                            </div>
                        </div>
                    </div>

                    {/* Right Column: Form */}
                    <div className="bg-[#0b0c15] rounded-3xl border border-white/10 p-8 shadow-2xl">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Book Title</label>
                                <input
                                    type="text"
                                    value={data.book_title}
                                    onChange={e => setData('book_title', e.target.value)}
                                    placeholder="e.g. The Future of AI"
                                    className={`w-full bg-[#1c1f2e] border border-white/10 rounded-xl px-4 py-4 text-white placeholder-gray-600 focus:outline-none ${focusBorder} transition-all`}
                                />
                                {errors.book_title && <p className="text-red-400 text-xs mt-1">{errors.book_title}</p>}
                            </div>
                            ...
                            <div className="pt-6">
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className={`w-full py-4 ${buttonClass} text-white font-black text-lg rounded-xl transition-all transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}
                                >
                                    {processing ? (
                                        <>
                                ...
                                <p className="text-center text-gray-500 text-xs mt-4">By continuing, you agree to our Terms of Service.</p>
                            </div>
                        </form>
                    </div>
