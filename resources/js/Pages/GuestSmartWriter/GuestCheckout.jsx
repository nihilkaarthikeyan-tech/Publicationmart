...
export default function GuestCheckout({ plan, price, pages }) {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-parchment text-ink font-sans flex items-center justify-center p-4">
                ...
                <div className="w-full max-w-5xl relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-start">
                    ...
                    <div className="space-y-8 lg:sticky lg:top-10">
                        <div>
                            <h1 className="text-4xl font-black text-ink mb-2 tracking-tight">Setup Your Book</h1>
                            <p className="text-umber">Enter your details to create your writing workspace.</p>
                        </div>

                        <div className="bg-paper rounded-3xl border border-linen p-8 relative overflow-hidden group">
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-umber text-sm font-medium uppercase tracking-wider">Selected Plan</h3>
                                {isPremium && (
                            ...
                            <div className="flex justify-between items-end mb-6">
                                <div>
                                    <h2 className="text-3xl font-bold text-ink capitalize">{plan} Plan</h2>
                                    <p className={`${accentText} font-medium`}>{pages} Pages</p>
                                </div>
                                <div className="text-right">
                                    <span className="text-3xl font-black text-ink">₹{Number(price).toLocaleString('en-IN')}</span>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-linen pt-6">
                                <ul className="space-y-3">
                                    {features.map((f, idx) => (
                                        <li key={idx} className="flex items-center gap-3 text-sm text-ink-soft">
                        ...
                        <div className={`${glowBg} border ${glowBorder} rounded-2xl p-6 flex gap-4 items-start`}>
                            ...
                            <div>
                                <h4 className="font-bold text-ink text-sm mb-1">Secure Payment via PhonePe</h4>
                                <p className="text-umber text-xs leading-relaxed">Your payment is processed securely. We do not store your card details. Redirecting to secure gateway upon confirmation.</p>
                            </div>
                        </div>
                    </div>

                    {/* Right Column: Form */}
                    <div className="bg-paper rounded-3xl border border-linen p-8 shadow-2xl">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-umber uppercase tracking-widest mb-2">Book Title</label>
                                <input
                                    type="text"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-xs font-bold text-umber uppercase tracking-widest mb-2">Full Name</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-umber uppercase tracking-widest mb-2">Email Address</label>
                                    <input
                                        type="email"
                                        value={data.email}
                            ...
                            <div className="pt-6">
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className={`w-full py-4 ${buttonClass} text-ink font-black text-lg rounded-xl transition-all transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}
                                >
                                    {processing ? (
                                        <>
                                            <svg className="animate-spin h-5 w-5 text-ink" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                            Processing...
                                        </>
                                    ) : (
                                ...
                                <p className="text-center text-umber text-xs mt-4">By continuing, you agree to our Terms of Service.</p>
                            </div>
                        </form>
                    </div>
