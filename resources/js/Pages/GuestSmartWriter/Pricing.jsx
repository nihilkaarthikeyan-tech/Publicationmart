            ...
            <div className="min-h-screen bg-parchment text-ink font-sans selection:bg-indigo-500 selection:text-ink overflow-x-hidden">
                ...
                <section className="relative pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                    ...
                    <div className={`text-center mb-12 transition-all duration-1000 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        {/* Badge */}
                        <div className="inline-flex items-center gap-3 px-5 py-2 mb-8 rounded-full bg-paper border border-linen text-sm font-medium text-ink-soft">
                            <span className="text-[10px] font-black uppercase tracking-widest text-umber border-r border-linen pr-3">IN INDIA'S FIRST</span>
                        ...
                        <h1 className="text-5xl md:text-7xl font-black mb-4 tracking-tight leading-tight">
                            <span className="text-ink">Turn Ideas into Books</span>
                            <br />
                            <span className=" text-oxblood">— Automatically</span>
                        </h1>
                        <p className="text-lg text-umber max-w-2xl mx-auto">
                    ...
                    <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        ...
                        {/* PRO CARD */}
                        <div className="relative bg-white/[0.04] border border-linen rounded-3xl p-8 hover:border-taupe hover:bg-white/[0.06] transition-all duration-300 group">
                            ...
                            <button
                                onClick={scrollToPro}
                                className="w-full py-3.5 border border-linen text-ink font-bold rounded-xl hover:bg-vellum transition-all text-sm tracking-wider uppercase"
                            >
                                View Pro Plans →
                            </button>
                        </div>
                ...
                <section className="relative pb-24 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-4xl font-black mb-4"><span className="text-ink">From Idea to Manuscript</span> <span className="text-umber font-normal">in Minutes</span></h2>
                        <p className="text-umber">Here is how Smart Writer accelerates your process.</p>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto">
                        <div className="grid lg:grid-cols-2 gap-16 items-center">
                            ...
                            <div className="relative group">
                                ...
                                <div className="relative bg-paper border border-orange-500/30 rounded-3xl p-8 md:p-10">
                                    ...
                                    <p className="text-ink-soft mb-6 leading-relaxed">Smart Writer is designed to assist — not replace — the author. All AI-generated content serves as a foundation and must be:</p>
                                    <ul className="space-y-3 text-umber mb-8">
                ...
                {/* ═══ PRO PRICING GRID ═══ */}
                <div ref={proGridRef} className="pt-24 pb-16 relative z-10 border-t border-linen bg-paper">
                    <div className="max-w-7xl mx-auto px-6">
                        <div className="text-center mb-16">
                            ...
                            <h2 className="text-4xl font-black text-ink mb-4">Select Your Pro Plan</h2>
                            <p className="text-umber text-lg">Choose a plan based on your desired book length.</p>
                        </div>
                        ...
                        <div className="text-center mt-10">
                            <button onClick={scrollToPremium} className="text-sm text-umber hover:text-cyan-700 transition-colors underline underline-offset-4">
                ...
                <div ref={premiumGridRef} className="pt-16 pb-32 relative z-10 bg-paper">
                    <div className="max-w-7xl mx-auto px-6">
...
function TierFeature({ icon, title, desc, color = 'indigo' }) {
    ...
    let titleColor = 'text-ink';
    let descColor = 'text-umber';

    if (color === 'cyan') {
        checkColor = 'text-cyan-700';
    } else if (color === 'gray') {
        checkColor = 'text-umber';
        titleColor = 'text-umber';
        descColor = 'text-umber';
    }
...
function PricingCard({ plan, tier, accentClass }) {
    ...
    return (
        <div className={`relative flex flex-col group ${plan.popular ? 'lg:-translate-y-4' : ''}`}>
            ...
            <div className={`relative flex flex-col h-full bg-paper rounded-[20px] border ${borderColor} overflow-hidden transition-all duration-300 group-hover:border-${accentClass}-500/30 group-hover:transform group-hover:-translate-y-1 group-hover:shadow-2xl`}>
                <div className="p-8 pb-32 flex-1">
                    <h3 className="text-xl font-black text-ink mb-4 uppercase tracking-wider">{plan.name}</h3>
                    <div className="flex items-baseline gap-2 mb-2">
                        <span className="text-4xl font-black text-ink">₹{plan.price.toLocaleString('en-IN')}</span>
                        <span className="text-sm text-umber line-through">₹{plan.originalPrice.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="mb-8 p-3 bg-paper rounded-xl border border-linen">
                        <span className={`text-oxblood text-sm font-bold flex items-center gap-2`}>
                    ...
                    <ul className="space-y-4">
                        {plan.features.map((feature, idx) => (
                            <li key={idx} className="flex items-start gap-3">
                                ...
                                <span className="text-sm text-umber font-medium group-hover:text-ink-soft transition-colors">{feature}</span>
                            </li>
                        ))}
                    </ul>
                </div>
...
function StepCard({ number, title, subtitle, icon, side, delay, gradient, glowColor, children }) {
    ...
    return (
        <div ref={domRef} className={`flex flex-col md:flex-row items-start md:items-center relative group ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} transition-all duration-1000 ease-out`} style={{ transitionDelay: `${delay}ms` }}>
            <div className={`absolute left-6 md:left-1/2 w-12 h-12 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-ink font-black text-lg shadow-lg z-10 transform md:-translate-x-1/2 border-4 border-ink ring-2 ring-linen`}>{number}</div>
