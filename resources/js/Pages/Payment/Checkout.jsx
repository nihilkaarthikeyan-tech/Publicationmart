...
export default function Checkout({ auth, book, purchaseType = 'hardcover' }) {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-[#0a0b0f] text-white py-12">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    {/* Header */}
                    <div className="text-center mb-12">
                        <h1 className="text-4xl font-black text-white mb-2">Secure Checkout</h1>
                        <p className="text-gray-400">Complete your {purchaseType === 'audio' ? 'Audiobook' : 'Hardcover'} purchase</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="bg-[#15161b] rounded-2xl border border-white/10 p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <input type="text" name="full_name" value={form.full_name} onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Enter full name" required />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500 uppercase">WhatsApp Number</label>
                                        <input type="tel" name="phone" value={form.phone} onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="+91 99999 99999" required />
                                    </div>
                                    <div className="space-y-1 md:col-span-2">
                                        <label className="text-xs font-bold text-gray-500 uppercase">Email Address (For Invoice)</label>
                                        <input type="email" name="email" value={form.email} onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="name@example.com" required />
                                    </div>
                                    <div className="space-y-1 md:col-span-2">
                                        <label className="text-xs font-bold text-gray-500 uppercase">Full Address</label>
                                        <textarea name="address" value={form.address} onChange={handleChange} rows="2" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Flat No, Street, Landmark" required></textarea>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500 uppercase">City</label>
                                        <input type="text" name="city" value={form.city} onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="City" required />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500 uppercase">State</label>
                                        <input type="text" name="state" value={form.state} onChange={handleChange} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="State" required />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500 uppercase flex justify-between">
                                            <span>Pincode</span>
                                            {isLoadingPincode && <span className="text-indigo-400 animate-pulse">Checking...</span>}
                                        </label>
                                        <input
                                            type="text"
                                            name="pincode"
                                            ...
                                            maxLength="6"
                                            className={`w-full bg-white/5 border rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${pincodeError ? 'border-red-500 focus:ring-red-500' : 'border-white/10'}`}
                                            placeholder="000000"
                                            required
                                        />
                            ...
                            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-6 flex gap-4 items-start">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-white mb-1">Pay with PhonePe Secure Gateway</h2>
                                    ...
                                    <div className="flex gap-2 mt-3 opacity-60">
                                        <span className="bg-white/10 px-2 py-1 rounded text-[10px] uppercase">UPI</span>
                                        <span className="bg-white/10 px-2 py-1 rounded text-[10px] uppercase">Cards</span>
                                        <span className="bg-white/10 px-2 py-1 rounded text-[10px] uppercase">NetBanking</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-1">
                            <div className="bg-[#15161b] rounded-2xl border border-white/10 p-6 sticky top-8">
                                ...
                                <div className="flex gap-4 mb-6 pb-6 border-b border-white/10">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-indigo-400 text-xs mt-1 font-medium capitalize">{purchaseType} Edition</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3 mb-6">
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-400">Price ({purchaseType})</span>
                                        <span className="text-white">₹{basePrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                    </div>
                                    {/* Discount Show */}
                                    {discount > 0 && (
                                ...
                                <div className="mb-6">
                                    <div className="relative flex">
                                        <input
                                            ...
                                            className="w-full bg-white/5 border border-white/10 rounded-l-lg px-4 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 placeholder-gray-500 uppercase font-mono"
                                        />
                                        <button
                                            type="button"
                                    ...
                                    {couponMessage && (
                                        <p className={`text-xs mt-2 font-bold ${couponMessage.type === 'success' ? 'text-green-400' : 'text-red-400'}`}>
                                            {couponMessage.text}
                                        </p>
                                    )}
                                </div>
                                ...
                                <Link
                                    ...
                                    className="block text-center mt-4 text-gray-400 hover:text-white text-sm transition"
                                >
